...
var memoInputElements = {
  ...
  deadline : document.querySelector("#dead-line"),
  priority : document.querySelector("#memo-priority"),
  button: {
    ...
    sortPriority: document.querySelector("#priority-sort")}
};
...
/*
 メモオブジェクトを作成する関数
 */
var createMemo = function(title, place, details, picture, deadline, priority){
  return {
    title: title,
    place: place,
    details: details,
...
var createMemoPlaceElement = function(memo){
  var div = document.createElement("div");
  div.textContent = "場所：" + memo.place;
  div.setAttribute("class", "memo-place");
  return div;
};

/*優先度表示*/
var createMemoPriorityElement = function(memo){
  var div = document.createElement("div");
  div.textContent ="優先度：" + memo.priority;
  div.setAttribute("class", "memo-priority");
  return div;
};

/*締切表示 */
var createMemodeadElement = function(memo){
  var div = document.createElement("div");
  div.textContent = "締切：" + memo.deadline;
  div.setAttribute("class", "memo-deadline");
  return div;
};
/*
...
var createMemoElement = function(memo){
  ...
  div.appendChild(createMemodeadElement(memo));
  div.appendChild(createMemoPriorityElement(memo));
...
var clearMemoInput = function(){
  ...
  memoInputElements.deadline.value = "";
...
var addMemo = function(){
  var newMemo = createMemo(memoInputElements.title.value,
                           ...
                           memoInputElements.deadline.value,
                           memoInputElements.priority.value);
  console.log(newMemo);
  memos.push(newMemo);
  displayMemo(newMemo);
  saveMemoList();
...
var initApp = function(){
  ...
  memoInputElements.button.sortPriority.addEventListener("click", array);
